...
{
    [TestMethod]
    public void TestMaxGridDiskSize()
    {
        long size = 0;
        var error = H3.maxGridDiskSize(3, ref size);
        Assert.AreEqual(size, 37);
    }
    ...
    public void TestGridDisk()
    {
        ...
        int k = 3;
        long size = 0;
        var error = H3.maxGridDiskSize(k, ref size);
        ulong[] cells = new ulong[size];
        error = H3.gridDisk(cell, k, cells);
    ...
    public void TestGridPathCells()
    {
        ...
        ulong[] path = new ulong[size];
        error = H3.gridPathCells(origin, destination, path);
    ...
    public void TestCellToLocalIj()
    {
        ulong origin = 621923649824456703;
        ulong destination = 621923649821540351;
        CoordIJ coordIJ = new CoordIJ();
        var error = H3.cellToLocalIj(origin, destination, 0, ref coordIJ);
        Assert.AreEqual(coordIJ.i, -3985);
    }
}
